...
| `data_path` | `str` | `"data.json"` | компоненты `..` запрещены | Путь к JSON-файлу хранилища |
...
`path` преобразует `data_path` в `Path`; остальные свойства возвращают
разобранные часы и минуты соответствующего расписания.
...
| `registration_state` | `Literal["closed", "opening", "open"]` | Состояние жизненного цикла регистрации |
...
`ValueError`, который обработчик показывает пользователю.
...
### `help_text(*, compact: bool = False) -> str`

Возвращает статический текст справки по командам. Сейчас `compact=True`
сохраняет тот же текст и используется callback-кнопкой как явный режим вызова.
...
Создаёт JSON inline-клавиатуры, общей для текстовых ответов, анонса и
напоминания. Payload кнопок: `join`, `leave`, `list`, `help`.

---

## `src/__init__.py`

Маркер пакета `src`; публичных классов и функций не содержит.
...
Регистрирует 10 текстовых и 4 callback-хендлера. Общий wrapper `target_peer_only`
...
Payload (`join`, `leave`, `list`, `help`) составляют контракт между клавиатурой
и четырьмя callback-хендлерами.
